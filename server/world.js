...
function World(args) {
  ...
  const size = args.clusterSize;
  const clusterWidth = Math.floor(args.width / size);
  const clusterHeight = Math.floor(args.height / size);
  ...
  this.clusters = {};
  if (args.indexes) {
    for (let i = 0; i < args.indexes.length; i++) {
