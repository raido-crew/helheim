...
Cluster.prototype.updateItem = function(item) {
  ...
  item.x = Math.max(radius,
      Math.min(this.width * this.size - radius, item.x));
  item.y = Math.max(radius,
      Math.min(this.height * this.size - radius, item.y));

  var node = this.pick(item);
...
Cluster.prototype.ind = function(point) {
  return (Math.max(0,
      Math.min(this.width - 1, Math.floor(point[0] / this.size))) %
      this.width) + (Math.max(0,
      Math.min(this.height - 1, Math.floor(point[1] / this.size))) *
      this.width);
};
...
Cluster.prototype.forEachAround = function(item, range, fn) {
  ...
  for (var y = Math.max(0, node.y - range); y <=
  Math.min(this.height - 1, node.y + range); y++) {
    for (var x = Math.max(0, node.x - range); x <=
    Math.min(this.width - 1, node.x + range); x++) {
